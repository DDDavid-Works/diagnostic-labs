...
{
    /// <summary>
    /// Interaction logic for ImmunologyWindow.xaml
    /// </summary>
    public partial class ImmunologyWindow : Window
    {
        public ImmunologyWindow()
        {
            InitializeComponent();
            this.DataContext = new ImmunologyViewModel(0);
        }

        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            var vm = (ImmunologyViewModel)DataContext;
        ...
        {
            ShowSelectMultiLineEntryWindow(MultiLineEntries.ImmunologyResult);
        }
        ...
        {
            ShowSelectMultiLineEntryWindow(MultiLineEntries.ImmunologyRemarks);
        }
        ...
        private void ShowSelectMultiLineEntryWindow(string fieldName)
        {
            ...
            switch (fieldName)
            {
                case MultiLineEntries.ImmunologyResult:
                    ResultTextBox.Text = mlew.SelectedMultiLineEntry.FieldValue;
                    break;
                case MultiLineEntries.ImmunologyRemarks:
        ...
        {
            this.Title = isSetDefaults ? "Immunology [SET DEFAULT MODE]" : "Immunology";
        ...
        {
            SearchLabResultsWindow search = new SearchLabResultsWindow(Modules.Immunology);
            search.ShowDialog();

            if (search.SelectedLabResult == null) return;

            this.DataContext = new ImmunologyViewModel(search.SelectedLabResult.Id);
        }
        ...
        private void ActionToolbar_PrintCommand(object sender, RoutedEventArgs e)
        {
            ...
            if (vm.Immunology != null)
            {
                PrintWindow print = new PrintWindow(Modules.Immunology, vm.Immunology.Id);
                print.ShowDialog();
            }
        }
